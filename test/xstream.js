...
describe.only('Cyclic Router - XStream - ', () => {
  ...
  describe('define()', () => {
    it('should return a stream',
      () => {
        const history = createServerHistory()
        const router = makeRouterDriver(history)(xs.of('/'), XSAdapter).define({})
        assert.strictEqual(router instanceof Stream, true)
    ...
    it('should match routes against a definition object', done => {
      ...
      const history = createServerHistory()
      const router = makeRouterDriver(history)(xs.fromArray(routes), XSAdapter)
      const match$ = router.define(definition)
